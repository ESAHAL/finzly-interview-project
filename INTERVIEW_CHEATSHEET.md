...
7. **Ask this PDF (grounded Q&A)** — after an analysis, a Q&A panel lets the user ask free-form questions about the document. Mechanism: a dedicated `/api/ask` route sends the PDF *again* to Gemini together with the question, and the response is forced into a Zod schema with two fields: `foundInPdf: boolean` and `answer: string`. The prompt explicitly instructs the model: answer strictly from the PDF if possible; otherwise set `foundInPdf: false` and give a general-knowledge answer. The UI then renders two visually distinct states — a green-accented **"Answer · from the PDF"** card for grounded answers, or a red-accented **"Warning · not found in the PDF"** card that clearly states the answer is general AI knowledge, not from the document. Key talking points: (a) this is a *hallucination guardrail* — the boolean flag makes source attribution machine-checkable rather than relying on the model's phrasing; (b) the client keeps the last analysed source (URL string or the `File` object in React state) and re-sends it with each question, so the server stays stateless; (c) restoring an old analysis from session history hides the Q&A panel to prevent answering against the wrong document — a subtle consistency bug I deliberately guarded against.
