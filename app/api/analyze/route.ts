...
export async function POST(req: Request) {
  ...
  // ---- 3. Verify it is actually a PDF (magic bytes: "%PDF") ----
  const header = new Uint8Array(pdfBuffer.slice(0, 5))
  const headerText = new TextDecoder().decode(header)
  if (!headerText.startsWith('%PDF')) {
    return errorResponse('That URL does not point to a PDF file. Please provide a direct link to a PDF.', 415)
  }

  // ---- 4. Send the PDF to Gemini and request structured JSON output ----
  try {
